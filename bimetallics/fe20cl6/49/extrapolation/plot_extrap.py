...
for key in energy_var:
    ...
    for j in range(len(x)):
        ax.plot(x[j], y[j]+2.605249, marker='o', linestyle='-', markersize=8, color=cb[key+1])
        ax.plot(x[j], z[j]+2.605249, marker='x', linestyle=' ', markersize=8, color=cb[key+1])
    
    x2 = np.array([-1,0])*conversion
    line = m*x2+b+2.605249
    print("y axis values are", line)
    print("b value is ", b)
    ax.plot(x2, line, alpha=1.0, color=cb[key+1], linestyle='-', linewidth=1.5, label="S=%d" % key )
    line = m2*x2+b +2.605249  
    print("y axis values are", line)                                  
