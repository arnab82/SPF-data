...
function run_spt_scan()
    for geom in reverse(8:8)
        ...
        #redirect_stdio(stdout=f_out, stderr=f_err) do
            ...
            @load "data_spt_008.jld2"
	    v_var2=FermiCG.BSTstate(v_var;  R=9) 
            @time e2 = FermiCG.compute_pt2_energy(v_var2, cluster_ops, clustered_ham, thresh_foi=1e-6,verbose=2,prescreen   = true,compress_twice = true)
