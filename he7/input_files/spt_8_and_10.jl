...
function run_spt_scan()
    for geom in reverse(10:10)
        ...
        #redirect_stdio(stdout=f_out, stderr=f_err) do
            ...
            eguess, vguess = FermiCG.ci_solve(ci_vector, cluster_ops, clustered_ham,conv_thresh=5e-4)
            ...
            e_var, v_var = block_sparse_tucker(vguess, cluster_ops, clustered_ham,
                                   ...
                                   tol_tucker  = 1e-4,
                                   solver      = "davidson")
            @time e2 = FermiCG.compute_pt2_energy(v_var, cluster_ops, clustered_ham, thresh_foi=1e-6,verbose=2,prescreen   = true,compress_twice = true)
	    ...
	    print("*E2: ")
            ...
            @save @sprintf("/home/arnab22/SPF-data/he7/spt/data_spt_%03i.jld2",geom) vguess v_var e2  eguess e_var  e2guess 
        #end
    end
end
...
function run_spt_scan()
    for geom in reverse(8:8)
        ...
        #redirect_stdio(stdout=f_out, stderr=f_err) do
            ...
            display(ci_vector)
            FermiCG.eye!(ci_vector)
            eguess, vguess = FermiCG.ci_solve(ci_vector, cluster_ops, clustered_ham,conv_thresh=1e-2)
            @time e2guess = FermiCG.compute_pt2_energy(vguess, cluster_ops, clustered_ham, thresh_foi=1e-8);
                                   nbody       = 4,
                                   H0          = "Hcmf",
                                   thresh_var  = 1e-3,
