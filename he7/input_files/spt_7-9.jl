...
function run_spt_scan()
    for geom in reverse(7:9)
        ...
        #redirect_stdio(stdout=f_out, stderr=f_err) do
            ...
            e_var, v_var_geom = block_sparse_tucker(vguess, cluster_ops, clustered_ham,
                                   max_iter    = 20,
                                   nbody       = 4,
                                   H0          = "Hcmf",
                                   thresh_var  = 1e-3,
            ...
            @eval v_var_$geom = v_var_geom
            @save @sprintf("data_spt_%03i.jld2",geom) vguess v_var_geom  eguess e_var e2guess 
        #end
    end
end
...
@time e2 = FermiCG.compute_pt2_energy(v_var_9, cluster_ops, clustered_ham, thresh_foi=1e-5,prescreen   = true,compress_twice = true)
@time e2 = FermiCG.compute_pt2_energy(v_var_8, cluster_ops, clustered_ham, thresh_foi=1e-5,prescreen   = true,compress_twice = true)
@time e2 = FermiCG.compute_pt2_energy(v_var_7, cluster_ops, clustered_ham, thresh_foi=1e-5,prescreen   = true,compress_twice = true)
