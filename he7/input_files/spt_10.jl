...
function run_spt_scan()
    for geom in reverse(10:10)
        ...
        #redirect_stdio(stdout=f_out, stderr=f_err) do
            ...
            eguess, vguess = FermiCG.ci_solve(ci_vector, cluster_ops, clustered_ham,conv_thresh    = 5e-5)
            #@time e2guess = FermiCG.compute_pt2_energy(vguess, cluster_ops, clustered_ham, thresh_foi=1e-7);
	    ...
	    print("*EGuess: ")
            ...
            @save @sprintf("data_spt_%03i.jld2",geom) vguess v_var  eguess e_var e2  
        #end
    end
end
